...
to create project directly : <mkdir python-docker> this creates dedicated folder for docker and pyhton files 
...
if using normal terminal nano <filename.py> used 
<pwd> checks the directory 
<ls> lists the file
<docker run -it -v $(pwd):/usr/src/app my-python-app python file.py> this command was so long so just use <drun filename,py> we have to create alias for this short commands 
for that open normal terminal and add this <~/.bashrc> and add this line <alias drun='docker run -it -v $(pwd):/usr/src/app my-python-app python'> att the end  the save and enter 
